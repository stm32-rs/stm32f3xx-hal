    ...
    // I2C::Bus: GetBusFreq,
{
    ...
    fn _i2c(i2c: I2C, pins: PINS, mode: Mode, clocks: Clocks, apb: &mut APB1) -> Self {
        ...
        // let pclk1 = 72.hz(); // todo temp!!!

        // assert!(mode.get_frequency().0 <= 400_000);

        let pclk1 = 28_000_000; // todo temp!!
    ...
    I2C: Deref<Target = I2cRegisterBlock>,
{
    ...
    fn init(&mut self, apb1: &mut APB1, clocks: &Clocks) {
        ...
        let freq = 10_000; // todo temp!!

        // todo: do for i2c2 and 3.
        apb1.enr().modify(|_, w| w.pwren().enabled());
        ...
        let i2cclk = clocks.sysclk().0; // todo .0 ok?
    ...
    fn reset(&mut self, apb1: &mut APB1, clocks: &Clocks) {
        self.i2c.cr1.write(|w| w.pe().set_bit().swrst().set_bit());
        self.i2c.cr1.reset();
        self.init(apb1, clocks);
    }
